...
erpt.LR <- function(Phi, P, m, H, vx, vy, cumulative = FALSE) {
  if (cumulative == FALSE) {
    I <- sirf(Phi, P, m, H)
  } else {
    I <- sirf(Phi, P, m, H, cumulative = TRUE)
  }
  return(100 * (I[vy, vy, ] - I[vx, vy, ]) / I[vy, vy, ])
}
...
SVAR.erpt.LR <- function(SVAR, H, vx, vy) {
  RF <- VAR1.coef(SVAR$var, SVAR$var$K, SVAR$var$p)
  ERPT <- erpt.LR(RF$Phi, solve(SVAR$A, SVAR$B), SVAR$var$K, H, vx, vy, cumulative = TRUE)
  return(ERPT)
}