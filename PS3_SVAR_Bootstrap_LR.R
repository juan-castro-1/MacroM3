...
SVAR.sirf.boot.LR <- function(SVAR, Yb, pmax, H, a, R, cumulative = FALSE) {
...
SVAR.fevd.boot.LR <- function(SVAR, Yb, pmax, H, a, R) {
...
SVAR.erpt.boot.LR <- function(SVAR, Yb, pmax, H, vx, vy, a, R, cumulative = FALSE) {
  Eb <- array(NA, c(H + 1, R))
  nvars <- colnames(SVAR$var$y)
  for (r in 1:R) {
    VAR.rep.R <- boot.rb.VAR(nvars, Yb[, , r], SVAR$var$K, SVAR$var$p, pmax)
    VAR.rep.S <- BQ(VAR.rep.R)
    if (cumulative == FALSE) {
      Eb[, r] <- SVAR.erpt.LR(VAR.rep.S, H, vx, vy)
    } else {
      Eb[, r] <- SVAR.erpt.LR(VAR.rep.S, H, vx, vy, cumulative = TRUE)
    }
  }
  El <- array(NA, c(H + 1))
  Eu <- array(NA, c(H + 1))
  if (cumulative == FALSE) {
    Ec <- SVAR.erpt.LR(SVAR, H, vx, vy)
  } else {
    Ec <- SVAR.erpt.LR(SVAR, H, vx, vy, cumulative = TRUE)
  }
  for (h in 1:(H + 1)) {
